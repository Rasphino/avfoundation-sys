...
fn sdk_path(target: &str) -> Result<String, std::io::Error> {
    ...
    let sdk = match target {
        t if t.contains("ios") => {
            ...
            {
                "iphonesimulator"
            } else if vec!["aarch64-apple-ios", "armv7-apple-ios", "armv7s-apple-ios"].contains(&t)
            {
                "iphoneos"
            } else {
                unreachable!();
...
fn build(sdk_path: Option<&str>, target: &str) {
    ...
    bindings
        .write_to_file(out_dir.join("avfoundation.rs"))
...
fn main() {
    ...
    if !target.contains("apple") {
        panic!("avfoundation-sys requires the apple target");
    }
